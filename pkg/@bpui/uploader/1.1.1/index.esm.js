...
import { net, crypt as crypt$1, string, utils, dom } from 'febs-browser';
import Vue from 'vue';
...
function upload(cfg) {
  ...
  var uid = "febsuifile" + crypt$1.uuid();
  uid = string.replace(uid, "-", "");
  var is_IE9 = utils.browserIEVer() <= 9;
  ...
  if (is_IE9 && cfg.formObj) {
    ...
    if (cfg.checkoutCrc32) {
      crypt.crc32_fileSegment(file, 0, -1, function (crc) {
        if (crc) {
          uploadFile.bind(utils.mergeMap(this, {
            crc: crc
          }))();
        } else {
...
var script$1 = {
  ...
  mounted: function mounted() {
    ...
    this.canvasSize = dom.getViewPort();
    ...
    dom.addEventListener(window, "orientationchange", this.onresize, false);

    if (!utils.browserIsMobile()) {
      dom.addEventListener(window, "resize", this.onresize);
    }
  },
  beforeDestroy: function beforeDestroy() {
    dom.removeEventListener(window, "orientationchange", this.onresize, false);

    if (!utils.browserIsMobile()) {
      dom.removeEventListener(window, "resize", this.onresize);
    }
  },
  methods: {
    ...
    onClipMouseDown: function onClipMouseDown(e) {
      ...
      this.mouseClipPos = utils.mergeMap(this.clipPos); // let css = window.getComputedStyle(currentTarget);
    ...
    onClipHornMouseDown: function onClipHornMouseDown(e) {
      ...
      this.mouseHornClipPos = utils.mergeMap(this.clipPos);
      this.$timer.setInterval(this.onDrawImage, 30);
    },
...
var script$2 = {
  ...
  methods: {
    ...
    drawImage: function drawImage(imageBase64) {
      ...
      this.canvasSize = dom.getViewPort(); // this.canvasSize.width *= 1;
    ...
    onClipMouseDown: function onClipMouseDown(e) {
      ...
      this.mouseClipPos = utils.mergeMap(this.clipPos);
      this.mouseImgOffset = utils.mergeMap(this.imgOffset); // let css = window.getComputedStyle(currentTarget);
